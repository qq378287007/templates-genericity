...
int main()
{
	...
	/*cout << ns1::funcsum<__int64>(&myintarray1[0], &myintarray1[2]) << endl;
	cout << ns1::funcsum<__int64>(&myintarray2[0], &myintarray2[2]) << endl;
	cout << (int)(ns1::funcsum<int>(&mychararray[0], &mychararray[2])) << endl;*/

	/*
	ns2::display_category(vector<int>::iterator());
	ns2::display_category(list<int>::iterator());*/

	/*cout << "vector<double>的元素类型为：" << typeid(ns3::GetEleType< vector<double> >::type).name() << endl;
	cout << "list<int>的元素类型为：" << typeid(ns3::GetEleType< list<int> >::type).name() << endl;
	cout << "float[45]的元素类型为：" << typeid(ns3::GetEleType< float[45] >::type).name() << endl;
	cout << "float[45]的数组元素数量为：" << ns3::GetEleType< float[45] >::size << endl;*/

	/*
	vector<double> mydblvec;
	ns3::PrintEleType(mydblvec);

	list<int> myintlist;
	ns3::PrintEleType(myintlist);

	float myfloatarr[45];
	ns3::PrintEleType(myfloatarr);
	...
	*/
	...
	ns4::print_is_same<decltype(a), decltype(b)>();
	ns4::print_is_same<decltype(a), decltype(c)>();*/

	// int&& a2 = 12;
	// ns4::RemoveReference_t<decltype(a2)> b2 = "dd";//从报错的结果就能看到b2的类型已经转换成了int类型

	/*
	int&& a2 = 12;
	ns4::RemoveReference_t<decltype(a2)> b2 = 125;
	int i = 64;
	int& c2 = i;
	ns4::RemoveReference_t<decltype(c2)> d2 = 500;
	using boost::typeindex::type_id_with_cvr;
	...
	*/
	...
	/*int anew = 15;
	ns5::AddLValueReference_t<decltype(anew)> bnew = anew; //decltype(anew)的类型为int类型，bnew的类型为int &
	int&& anew2 = 16;
	ns5::AddLValueReference_t<decltype(anew2)> bnew2 = anew2; //decltype(anew)的类型为int&&类型,bnew2的类型为int &
	*/

	/*
	int argnew = 15;
	ns5::AddRValueReference_t<decltype(argnew)> brgnew = 18; //brgnew的类型为int &&

	int&& argnew2 = 15;
	ns5::AddRValueReference_t<decltype(argnew2)> brgnew2 = 18;//brgnew的类型为int &&，根据引用折叠规则，两个右值引用遇到一起，结果还是右值引用

	int tmpvalue = 16;
	int& argnew3 = tmpvalue;
	ns5::AddRValueReference_t<decltype(argnew3)> brgnew3 = tmpvalue;  //brgnew3的类型为int &，因为根据引用折叠规则，左值引用和右值引用遇到一起，结果是左值引用
	*/

	/*
	ns6::RemoveConst_t<const int> nca = 15;  //nca是int类型
	nca = 18;  //可以给nca重新赋值*/
	...
	ns7::myffunc(c);
	ns7::myffunc(arry);
	ns7::myffunc(ns7::testFunc);


	std::decay<const int&>::type nb = 26;
	cout << "nb的类型为：" << typeid(decltype(nb)).name() << endl;  //nb的类型为int类型

	ns7::RemoveCR_t<const int&&> rcrobj = 16;  //rcrobj为int类型
	*/

	/*
		int arr[2] = { 1,2 };
		ns7::Decay<decltype(arr)>::type myarray = nullptr;
		cout << "myarray类型为：" << typeid(decltype(myarray)).name() << endl;

		extern int g_array[];
		ns7::Decay<decltype(g_array)>::type myarray2 = nullptr;
		cout << "myarray2类型为：" << typeid(decltype(myarray2)).name() << endl;


		void (*p)() = ns7::testFunc2; //定义一个函数指针，指向testFunc2函数
		p(); //调用p指向的函数，也就是testFunc2函数
		*/

	/*
		//ns7::Decay<decltype(ns7::testFunc2)>::type rfunc;
		ns7::Decay_t<decltype(ns7::testFunc2)> rfunc;
		cout << "rfunc类型为：" << typeid(decltype(rfunc)).name() << endl;
		rfunc = ns7::testFunc2;  //给函数指针赋值
		rfunc();  //调用rfunc函数
	*/
